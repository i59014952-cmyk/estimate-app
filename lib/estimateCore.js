...
async function submitLemanaBatch(names, city = 'kazan') {
    ...
    for (let i = 0; i < uniq.length; i += 50) {   // /lemana/jobs принимает до 50 запросов
        ...
        try {
            await fetch(`${PRICES_BACKEND}/lemana/jobs`, {
                method: 'POST',
                headers: { 'Content-Type': 'application/json' },
                body: JSON.stringify({ queries: chunk, city, limit_per_query: 5 }),
            });
...
Object.assign(window, {
    ...
    searchPriceCandidates, searchActiveStores, searchBackendCandidates, searchLemana, submitLemanaBatch,
