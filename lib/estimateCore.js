...
function readXlsx(arrayBuffer) {
    ...
    for (let R = range.s.r; R <= range.e.r; R++) {
        ...
        for (let C = range.s.c; C <= range.e.c; C++) {
            ...
            if (cell) {
                row.push(cell.v != null ? String(cell.v) : (cell.w != null ? String(cell.w) : ''));
...
function isColoredFill(cell) {
    ...
    const rgb = color.rgb;
    if (!rgb) return false;
    const upper = String(rgb).toUpperCase().replace(/^FF/, '');
    if (upper === 'FFFFFF' || upper === '' || upper === '000000' && fill.patternType === 'none') return false;
    return true;
}
