...
// Сначала по единице измерения (час/смена → работа), затем по ключевым
// словам в названии; иначе считаем материалом (их большинство в каталоге).
const WORK_NAME_RE = /(монтаж|демонтаж|установк|укладк|устройств|сборк|разборк|оштукатур|штукатурн|шпакл|шпатл|окрас|покрас|малярн|облицов|кладк|бетонир|армир|сверлен|бурен|стяжк|выравниван|утеплен|погрузк|разгрузк|пусконал|наладк|подключен|замер|обработк|распиловк|сварочн|сварк|работа|работы|работ\b|услуг)/i;
...
function classifyItem(name, unit) {
    ...
    if (u && WORK_UNIT_RE.test(u)) return 'work';
    if (WORK_NAME_RE.test(normalizeYo(String(name || '')))) return 'work';
    return 'material';
}

// Определяет категорию по тексту заголовка-раздела файла («Работы», «Материалы»).
// Возвращает null для неоднозначных («Работы и материалы») — категория не меняется.
...
// Объединяет результаты встроенных каталогов и активных магазинов пользователя.
// Встроенный поиск — основной (может бросить ошибку для логики ретраев),
// магазины добавляются best-effort.
async function searchPriceCandidates(name, limit = 6) {
    const storesPromise = searchActiveStores(name, limit).catch(() => []);
    const base = await searchBackendCandidates(name, limit);
    const stores = await storesPromise;
    if (!stores.length) return base;
    const merged = base.slice();
    const seen = new Set(base.map(x => x.url || x.name));
...
Object.assign(window, {
    ...
    searchPriceCandidates, searchActiveStores, searchBackendCandidates,
    classifyItem, detectSectionCategory,
});
