...
const GOODS_UNIT_RE = /^(шт|компл|комплект|к-т|кт|рул|рулон|лист|упак|уп|пар|пара|набор)\.?$/i;
...
// Поиск по Лемана ПРО (бэкенд /lemana/search, реальный браузер за Qrator).
// Best-effort: медленный/блокированный ответ не валит общий поиск.
async function searchLemana(name, limit = 6) {
    const TIMEOUT_MS = 25000;
    const ctrl = new AbortController();
    const t = setTimeout(() => ctrl.abort(), TIMEOUT_MS);
    try {
        const u = `${PRICES_BACKEND}/lemana/search?query=${encodeURIComponent(name)}&limit=${limit}`;
        const r = await fetch(u, { signal: ctrl.signal });
        if (!r.ok) return [];
        const j = await r.json();
        return (j.results || []).filter(x => x && x.price).map(x => ({
...
// Объединяет результаты встроенных каталогов, Лемана ПРО и активных магазинов.
// Встроенный поиск — основной (может бросить ошибку для логики ретраев),
// Лемана и магазины добавляются best-effort.
async function searchPriceCandidates(name, limit = 6) {
    const storesPromise = searchActiveStores(name, limit).catch(() => []);
    const lemanaPromise = searchLemana(name, limit).catch(() => []);
    const base = await searchBackendCandidates(name, limit);
    const extra = (await lemanaPromise).concat(await storesPromise);
...
Object.assign(window, {
    ...
    searchPriceCandidates, searchActiveStores, searchBackendCandidates, searchLemana,
