...
(function () {
  ...
  async function refresh() {
    ...
    let r;
    try {
      r = await fetch(`${API_BASE}/auth/refresh`, {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ refresh_token: session.refresh_token }),
      });
    } catch (_) {
      // Сеть/таймаут (например, сервер занят парсингом Lemana) — НЕ разлогиниваем,
      // сессию сохраняем, повторим позже.
      scheduleRetry();
      return false;
    }
    // Разлогиниваем ТОЛЬКО при настоящем отказе авторизации (невалидный/просроченный
    // refresh_token). Временный сбой сервера (5xx и пр.) — сессию не трогаем, повторяем.
    if (r.status === 401 || r.status === 403) { save(null); return false; }
    const data = await r.json().catch(() => ({}));
    if (!r.ok || !data.access_token) { scheduleRetry(); return false; }
    save({
      access_token: data.access_token,
      refresh_token: data.refresh_token,
      expires_at: data.expires_at || (Math.floor(Date.now() / 1000) + 3600),
  ...
  // Повторная попытка продлить токен после временного сбоя (через 20с).
  function scheduleRetry() {
    if (refreshTimer) { clearTimeout(refreshTimer); refreshTimer = null; }
    if (!session || !session.refresh_token) return;
    refreshTimer = setTimeout(() => { refresh().catch(() => {}); }, 20_000);
  }
  ...
  window.KHAuth = {
    // Залогинен, пока есть сессия с refresh_token — даже если access-токен на
    // миг просрочен (его обновит refresh). На экран входа выбрасываем только
    // когда сессии нет совсем или refresh окончательно отклонён (401/403).
    isAuthed: () => !!(session && session.refresh_token),
