...
function useEstimate() {
  ...
  const pollLemanaJobs = React.useCallback(async (jobs) => {
    ...
    while (Date.now() < deadline) {
      ...
      // Все наши позиции уже получили цену (спиннеров не осталось) — ждать
      // завершения серверной задачи незачем: она сама дойдёт до конца, кэш
      // сохранится. Иначе индикатор «Поиск цен в фоне…» висел бы, пока бэкенд
      // домалывает медленный парсинг, хотя смотреть уже не на что.
      if (!estimateRef.current.some(r => r.lemanaPending)) break;
